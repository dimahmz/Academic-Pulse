...
@Composable
fun ConfirmEmailPage(viewModel: AuthViewModel) {
    ...
    Column(
        ...
        Column(modifier = Modifier.padding(bottom = 30.dp, top=60.dp).fillMaxWidth(),
            ...
            // confirmation_error
            if(showErrorMsg)
                Title(R.string.confirmation_error , color = MaterialTheme.colorScheme.error)
        }


        Spacer(Modifier.weight(1f))
        H1(modifier=Modifier.padding(vertical = 25.dp).fillMaxWidth() ,text=R.string.resend_code, color = MaterialTheme.colorScheme.primary, align = TextAlign.Center)
...
fun PreviewConfirmEmailPage() {
    ConfirmEmailPage(AuthViewModel())
}
